...
saveButton.addEventListener('click', saveTypingResults);

function saveTypingResults() {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/save-typing-results', true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            console.log('Typing results saved successfully.');
        } else {
            console.error('Error saving typing results.');
        }
    };
    ...
    const typingResults = {
        ...
        errors: parseInt(errorTag.innerText),
    };

    console.log(typingResults);

    xhr.send(JSON.stringify(typingResults));
}